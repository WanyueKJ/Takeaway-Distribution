    ...
    */
    public function apppay($orderid, $money, $url, $body = '充值虚拟币')
    {
        ...
        if ($appid == "" || $key == "" || $publickey == "") {
            throw new ApiException('支付宝未配置');
        }

        $noceStr = md5(rand(100, 1000) . time());//获取随机字符串
        $time = time();

        require_once API_ROOT . '/../sdk/alipay/aop/AopClient.php';
        require_once API_ROOT . '/../sdk/alipay/aop/request/AlipayTradeAppPayRequest.php';
    ...
    public function aliAppRefund($outTradeNo, $refundAmount, $outRequestNo, $refundReason)
    {
        ...
        require_once API_ROOT . '/../sdk/alipay/aop/SignData.php';
        require_once API_ROOT . '/../sdk/alipay/aop/AopClient.php';
        require_once API_ROOT . '/../sdk/alipay/aop/request/AlipayTradeRefundRequest.php';
        ...
        if (!empty($resultCode) && $resultCode != 10000) {
            throw new ApiException($response->$responseNode->sub_msg);
        }
