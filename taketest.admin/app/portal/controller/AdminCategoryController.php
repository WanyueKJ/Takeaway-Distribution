...
<tr class='data-item-tr'>
    <td>
        <input type='checkbox' class='js-check' data-yid='js-check-y' data-xid='js-check-x' name='ids[]'
               value='\$id' data-name='\$name' \$checked lay-skin='primary'>
    </td>
    <td>\$id</td>
    <td>\$spacer \$name</td>
</tr>
tpl;
        ...
        if($type == 2){
            ...
            $where[] = ['delete_time', '=', 0];
            $where[] = ['special', '=', 1];
            $category = $portalCategoryModel->where($where)->select()->toArray();
            $inIds = array_column($category,'id') ?: [];
        }
        //<td>\$spacer <a href='\$url' target='_blank'>\$name</a></td>
        $categoryTree = $portalCategoryModel->adminCategoryTableTree($selectedIds, $tpl,$inIds);
